...
val backwardShips = shipList.map( (ship: String) => {ship.reverse});System.out.println("""backwardShips  : List[String] = """ + $show(backwardShips ));$skip(46); 
...
val scalaList = List.range(1,20);System.out.println("""scalaList  : List[Int] = """ + $show(scalaList ));$skip(19); 
println(scalaList);$skip(66); 
val divByThree = scalaList.filter( (x : Int) => Math.abs(x%3)==0);System.out.println("""divByThree  : List[Int] = """ + $show(divByThree ));$skip(103); 



// Map function applies the function to each and every value.
val listRDD = scalaList.map(x => x*x);System.out.println("""listRDD  : List[Int] = """ + $show(listRDD ));$skip(64); 
...
{
  x*x
};System.out.println("""squareIt: (x: Int)Int""");$skip(40); 

val listRDD2 = scalaList.map(squareIt);System.out.println("""listRDD2  : List[Int] = """ + $show(listRDD2 ))}
